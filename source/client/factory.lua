...
function factory:update(delta, input, game)
	...
	if input.interact.down then
		local mouseX, mouseY = math.floor(input.mouse.x/2 + self.camera.x), math.floor(input.mouse.y/2 + self.camera.y)
		if self.grid.tiles[mouseX] and self.grid.tiles[mouseX][mouseY] then
			local window = gui.tile(self.grid.tiles[mouseX][mouseY])
			if window then
				game:open(window)
			end
		end
